...
def gaussianRNB():
    ...
    cb.fit(X, d)
    end_time = time.time()
    tauxT = tauxRec_base_test(cb)
    tauxA = tauxdRec_base_appr(cb,X,d)
    print("\n le taux de connaissance a "
          "base d'apprentissage du"
          " gaussian est : ",tauxA," %")
    return cb ,end_time - start_time,tauxT

# apprentissage par le classifier MultinomialNB
def multinomialRNB():
    cb = MultinomialNB(alpha=0.2);
    X, d = X_D()
    start_time = time.time()
    cb.fit(X, d)
    end_time = time.time()
    ...
    print("\n le taux de connaissance"
          ...
          "Multinomial est : ", tauxA, " %")
    return cb, end_time - start_time,tauxT

# apprentissage par le classifier BernoulliNB
def bernoulliRNB():
    cb = BernoulliNB(alpha=0.2);
    X,d = X_D()
    start_time = time.time()
    cb.fit(X, d)
    end_time = time.time()
